// directions.jsx — the three identity directions as token bundles, plus the
// resolver that folds the finer Tweaks on top. Each direction is a complete
// design language: palette, type, shape, motion, motif. resolveTweaks(t) returns
// the CSS-variable map applied to .tally + the data-attributes that drive the
// direction-specific personality rules in tally.css.
...
const DIRECTIONS = {
  ...
  B: {
    ...
    tokens: {
      ...
      '--border': '#372d46',
      '--accent': '#ef9079',
      '--accent-contrast': '#2a1620',
      '--danger': '#e57a6c',
      '--font-display': "'Bricolage Grotesque', system-ui, sans-serif",
      '--font-body': "'Inter', system-ui, sans-serif",
      '--font-mono': "'Inter', system-ui, sans-serif",
      '--font-num': "'Bricolage Grotesque', system-ui, sans-serif",
      '--card-shadow': '0 12px 34px rgba(0,0,0,0.42)',
      '--card-border': '1px solid var(--border)',
    },
...
// Curated accent options (the first is "auto" = direction-native).
...
// Calm, nature-drawn full-color schemes. Recolor any direction (type/shape stay).
// Each is a complete token set — i.e. exportable color tokens for handoff.
const PALETTES = {
  auto: null,
  // ---- light ----
  ...
  // ---- monochrome / tonal (mainly shades) ----
...
// Heatmap palette presets (besides the direction-native one).
const HEAT_PRESETS = {
  auto: null, // use direction's own
...
const RADIUS_MODE = { sharp: 3, regular: null /* direction */, soft: 22 };
const TYPEFACE = {
  auto: null,
...
// Fold tweaks → { vars, attrs }
function resolveTweaks(t) {
  const dir = DIRECTIONS[t.direction] || DIRECTIONS.A;
  const vars = { ...dir.tokens };
  let dark = dir.dark;

  // palette: recolor the direction (type/shape stay). Sets its own dark flag + heat.
...
// Build a copyable :root{} CSS block from resolved tokens (handoff spec).
function tokensToCSS(t) {
  const { vars, dir, heat } = resolveTweaks(t);
  const pal = t.palette && t.palette !== 'auto' ? PALETTES[t.palette] : null;
  const lines = [];
  lines.push(`/* Tally — "${dir.name}" direction${pal ? ` · ${pal.label} palette` : ''} · generated design tokens */`);
  lines.push(':root {');
  const order = ['--bg','--surface','--surface-2','--text','--text-muted','--border',
    '--accent','--accent-contrast','--danger'];
  order.forEach(k => { if (vars[k]) lines.push(`  ${k}: ${vars[k]};`); });
  lines.push('');
  heat.forEach((c,i) => lines.push(`  --heat-${i}: ${c};`));
  lines.push('');
  ['--font-display','--font-body','--font-mono'].forEach(k => lines.push(`  ${k}: ${vars[k]};`));
  lines.push('');
  lines.push(`  --radius: ${vars['--radius']};`);
  lines.push(`  --density: ${vars['--density']};`);
  lines.push(`  --transition: ${vars['--t']};`);
  lines.push(`  --card-shadow: ${vars['--card-shadow']};`);
  lines.push('}');
  return lines.join('\n');
}

Object.assign(window, { DIRECTIONS, ACCENT_SWATCHES, PALETTES, HEAT_PRESETS, DENSITY, RADIUS_MODE, TYPEFACE, resolveTweaks, tokensToCSS });

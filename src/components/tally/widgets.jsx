// Presentational widgets ported from the handoff (components.jsx), 1:1 on markup/classes
// so tally.css styles them exactly as the prototype. Data comes from proto-adapters; the
...
// Lucide outline icon when the habit has one, else an "illuminated" serif monogram.
export function Glyph({ habit, size = 22 }) {
  const Icon = iconComponent(habit.iconName)
  if (Icon) return createElement(Icon, { size, strokeWidth: 2.1, 'aria-hidden': 'true' })
  return <span className="hcard__mono">{initial(habit.name)}</span>
}

export function TallyMark({ count = 5, h = 22, w = 2.5, style }) {
  const groups = []
  let rem = count
...
export function StreakBadge({ n, dim }) {
  if (n <= 0) return null
  return (
...
export function WeekDots({ cells }) {
  return (
    <div className="week">
      {cells.map((c) => (
...
export function CountControl({ value, goal, unit, onSet }) {
  const v = value || 0
  const done = v >= goal
  return (
...
export function TimerControl({ value, goal, unit, onSet }) {
  const v = value || 0
  ...
  useEffect(() => {
    ...
    ref.current = setInterval(() => setNow(performance.now()), 1000)
    return () => clearInterval(ref.current)
  }, [startedAt])
  const sessionMin = startedAt != null ? (now - startedAt) / 60000 : 0
  const shown = v + Math.max(0, sessionMin)
  const done = shown >= goal
  const pct = Math.min(100, (shown / goal) * 100)
  const running = startedAt != null
  const sessSecs = Math.max(0, Math.floor(sessionMin * 60))
  const mmss = `${Math.floor(sessSecs / 60)}:${String(sessSecs % 60).padStart(2, '0')}`
  const toggle = () => {
    if (!running) { const t = performance.now(); setStartedAt(t); setNow(t) }
    else { const mins = (performance.now() - startedAt) / 60000; setStartedAt(null); onSet((v || 0) + mins) }
  }
  ...
  return (
    <div className={'timer' + (done ? ' is-complete' : '')}>
      <div className="timer__head">
        ...
        {running && <span className="timer__goal" style={{ color: 'var(--accent)', fontVariantNumeric: 'tabular-nums' }}>● {mmss}</span>}
      </div>
      ...
      <div className="timer__actions">
        <button className={'timer__primary' + (running ? ' is-running' : '')} type="button" onClick={toggle}>
          {running ? 'Pause' : v > 0 ? 'Resume' : 'Start'}
        </button>
...
export function YearHeatmap({ grid, cell = 11, gap = 3 }) {
...
export function Sparkline({ series, color, w = 84, h = 26 }) {
